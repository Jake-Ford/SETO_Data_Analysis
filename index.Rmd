...
Variables <- c("Signed up", "Not signed up", 
          "Savings Rate 5%", "Savings Rate 10%", "Savings Rate 20%",
          "Cancellation Fee $0","Cancellation Fee $250",
          "1-year contract", "25-year contract",
          "Source = Qualtrics", "Source = Community Survey",
          "On No Gov't programs", "On at least 1 Gov't Program",
          
          "Renter", "Owner",
          "White", "Black", "Hispanic", "Asian", "Other - POC", 
          
          "Low Income", "Moderate Income", "High Income")


desc_temp <- data.frame(Variables)


desc_temp$N<- NA
desc_temp$Percent <- NA


### N
desc_temp$N[1] <- sum(survey$y_signup=="Yes", na.rm=T)
desc_temp$N[2] <- sum(survey$y_signup=="No", na.rm=T)

desc_temp$N[3] <- sum(survey$x_sr=="5% savings rate", na.rm = T)
desc_temp$N[4] <- sum(survey$x_sr=="10% savings rate", na.rm = T)
desc_temp$N[5] <- sum(survey$x_sr=="20% savings rate", na.rm = T)


desc_temp$N[6] <- sum(survey$x_cf=="0", na.rm=T)
desc_temp$N[7] <- sum(survey$x_cf=="250", na.rm=T)

desc_temp$N[8] <- sum(survey$x_cly=="1-year contract", na.rm = T)
desc_temp$N[9] <- sum(survey$x_cly=="25-year contract", na.rm = T)

desc_temp$N[10] <- sum(survey$x_source=="01 Qualtrics Panel", na.rm = T)
desc_temp$N[11] <- sum(survey$x_source=="02 Community", na.rm = T)

desc_temp$N[12] <- sum(survey$x_gov=="01 None, Unknown, or Other", na.rm = T)
desc_temp$N[13] <- sum(survey$x_gov=="02 At least one gov't program", na.rm = T)

desc_temp$N[12] <- sum(survey$x_gov=="01 None, Unknown, or Other", na.rm = T)
desc_temp$N[13] <- sum(survey$x_gov=="02 At least one gov't program", na.rm = T)

desc_temp$N[14] <- sum(survey$x_hs=="01 Homeowner", na.rm = T)
desc_temp$N[15] <- sum(survey$x_hs=="02 Renter", na.rm = T)

desc_temp$N[16] <- sum(survey$x_race=="01 White", na.rm = T)
desc_temp$N[17] <- sum(survey$x_race=="02 Black", na.rm = T)
desc_temp$N[18] <- sum(survey$x_race=="03 Hispanic", na.rm = T)
desc_temp$N[19] <- sum(survey$x_race=="04 Asian", na.rm = T)
desc_temp$N[20] <- sum(survey$x_race=="05 Other POC", na.rm = T)

desc_temp$N[21] <- sum(survey$x_inc=="03 Low income", na.rm = T)
desc_temp$N[22] <- sum(survey$x_inc=="02 Moderate income", na.rm = T)
desc_temp$N[23] <- sum(survey$x_inc=="01 High", na.rm = T)

### Percent

desc_temp$Percent[1] <- sum(survey$y_signup=="Yes", na.rm=T)/2986
desc_temp$Percent[2] <- sum(survey$y_signup=="No", na.rm=T)/2986

desc_temp$Percent[3] <- sum(survey$x_sr=="5% savings rate", na.rm = T)/2986
desc_temp$Percent[4] <- sum(survey$x_sr=="10% savings rate", na.rm = T)/2986
desc_temp$Percent[5] <- sum(survey$x_sr=="20% savings rate", na.rm = T)/2986


desc_temp$Percent[6] <- sum(survey$x_cf=="0", na.rm=T)/2986
desc_temp$Percent[7] <- sum(survey$x_cf=="250", na.rm=T)/2986

desc_temp$Percent[8] <- sum(survey$x_cly=="1-year contract", na.rm = T)/2986
desc_temp$Percent[9] <- sum(survey$x_cly=="25-year contract", na.rm = T)/2986

desc_temp$Percent[10] <- sum(survey$x_source=="01 Qualtrics Panel", na.rm = T)/2986
desc_temp$Percent[11] <- sum(survey$x_source=="02 Community", na.rm = T)/2986

desc_temp$Percent[12] <- sum(survey$x_gov=="01 Percentone, Unknown, or Other", na.rm = T)/2986
desc_temp$Percent[13] <- sum(survey$x_gov=="02 At least one gov't program", na.rm = T)/2986

desc_temp$Percent[12] <- sum(survey$x_gov=="01 Percentone, Unknown, or Other", na.rm = T)/2986
desc_temp$Percent[13] <- sum(survey$x_gov=="02 At least one gov't program", na.rm = T)/2986

desc_temp$Percent[14] <- sum(survey$x_hs=="01 Homeowner", na.rm = T)/2986
desc_temp$Percent[15] <- sum(survey$x_hs=="02 Renter", na.rm = T)/2986

desc_temp$Percent[16] <- sum(survey$x_race=="01 White", na.rm = T)/2986
desc_temp$Percent[17] <- sum(survey$x_race=="02 Black", na.rm = T)/2986
desc_temp$Percent[18] <- sum(survey$x_race=="03 Hispanic", na.rm = T)/2986
desc_temp$Percent[19] <- sum(survey$x_race=="04 Asian", na.rm = T)/2986
desc_temp$Percent[20] <- sum(survey$x_race=="05 Other POC", na.rm = T)/2986

desc_temp$Percent[21] <- sum(survey$x_inc=="03 Low income", na.rm = T)/2986
desc_temp$Percent[22] <- sum(survey$x_inc=="02 Moderate income", na.rm = T)/2986
desc_temp$Percent[23] <- sum(survey$x_inc=="01 High", na.rm = T)/2986
