...
raw <- read_excel(paste(input_folder, "3. Churn and Default/input/Central Hudson Preliminary Analysis/Nautilus Raw Data.xlsx", sep=""), 
    sheet = "final data")
...
churn_model <- glm(Churn ~ log_tenure + payment_method + kwh_solar  + Income + solar_farm, data = joined, family = "binomial")
default_model <- glm(Default ~ log_tenure + payment_method + kwh_solar  + Income + solar_farm, data = joined, family = "binomial")


stargazer(churn_model, default_model, title="Results", align=TRUE, type="html")
```
