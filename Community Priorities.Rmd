...
don't think RStata is going to be the way to do this - need to get the finished regression tables in Stata first
- first, get the regression tables formatted correctly in Stata, have those exported to maybe just an HTML for now
- next, copy and paste those ma-fuckers in here, we'll deal with what changes later
- finally, copy and paste those marginsplots and odds ratio graphs in here, keep it simple stupid
...
eststo: quietly logit y_signup i.x_sr i.x_race i.x_hs [pweight=weight_race_int], cluster(id)

eststo: quietly logit y_signup i.x_sr i.x_race  i.x_inc [pweight=weight_race_int], cluster(id)
...
eststo: quietly logit y_signup i.x_sr i.x_race i.x_inc i.x_hs i.x_clp i.x_cly i.x_cf i.x_fam  i.x_rev [pweight=weight_race_int], cluster(id)
